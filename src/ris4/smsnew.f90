...
!
      ...
      PRINT *, 'RIS4 SMSNEW: Using OpenMP with ', NUM_THREADS, ' threads'
!
...
!
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(IC, IR, CNUM, LCNUM, ITJPOC, I, VCOEFF, &
!$OMP& IIA, IIB, IIC, IID, K, LAB, J, LOC, CONTRIK1, CONTRI)
...
!
          IF (ITJPO(IR) .EQ. ITJPOC) THEN
            NVCOEF = 0
!   Critical section: RKCO_GG accesses global state
!$OMP CRITICAL(RKCO_CALC)
            CALL RKCO_GG (IC, IR, CORD, INCOR, 1)
!$OMP END CRITICAL(RKCO_CALC)
!
            DO 11 I = 1,NVCOEF
              VCOEFF = COEFF(I)
